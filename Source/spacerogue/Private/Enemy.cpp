...

{
	...
	//create left right collision boxes
...
void AEnemy::DoDamage(ASpaceRogueCharacter* Victim)
{
...
void AEnemy::SpawnBlood(ASpaceRogueCharacter* Victim, FName SocketName)
{
	const USkeletalMeshSocket* TipSocket{ Victim->GetMesh()->GetSocketByName(FName(TEXT("WoundSocket")))};
	if (TipSocket)
	{
		const FTransform SocketTransform{ TipSocket->GetSocketTransform(Victim->GetMesh()) };
...
void AEnemy::StunCharacter(ASpaceRogueCharacter* Victim)
{
	if (Victim)
	{
